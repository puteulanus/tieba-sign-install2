...
$("#zw_mailauth_clear").click(function(){
	createWindow().setTitle("清除已失效记录").setContent('你确定要清除authcode已经失效的待验证记录吗？').addButton("确定", function(){msg_callback_action('plugins/zw_mailauth/ajax.php?action=clear',zw_mailauth_load_set);}).addCloseButton("取消").append();
});	

$("#zw_mailauth_del_all").click(function(){
	createWindow().setTitle("全部删除").setContent('你确定要删除全部待验证记录吗？').addButton("确定", function(){msg_callback_action('plugins/zw_mailauth/ajax.php?action=alldel',zw_mailauth_load_set);}).addCloseButton("取消").append();
});	

$("#zw_mailauth_all_resend").click(function(){
	createWindow().setTitle("全部重发").setContent('你确定要重发所有验证邮件吗？').addButton("确定", function(){msg_callback_action('plugins/zw_mailauth/ajax.php?action=allresend',zw_mailauth_load_set);}).addCloseButton("取消").append();
});	

$("#zw_mailauth_all_pass").click(function(){
	createWindow().setTitle("全部通过").setContent('你确定要通过全部待验证记录吗？').addButton("确定", function(){msg_callback_action('plugins/zw_mailauth/ajax.php?action=allpass',zw_mailauth_load_set);}).addCloseButton("取消").append();
});	
...
function zw_mailauth_load_set(){
	showloading();
	$.getJSON("plugins/zw_mailauth/ajax.php?action=getsetting", function(result){
...
function zw_mailauth_operate(operate,id){
	...
	createWindow().setTitle(title).setContent(content).addButton('确定', function(){ msg_callback_action("plugins/zw_mailauth/ajax.php?action="+ajaxurl+"&id="+id,zw_mailauth_load_set); }).addCloseButton('取消').append();
	return false;
}
